Public Class FormPemasukanLainnya
    Private Sub LoadData()
        Try
            Dim sql As String = "SELECT kode_transaksi, nominal, keterangan, operator, tanggal_transaksi FROM transaksi WHERE jenis_transaksi = 'masuk' AND kode_tagihan IS NULL ORDER BY tanggal_transaksi LIMIT 20"
            Using conn = DatabaseConnector.GetConnection()
                Using cmd = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
                    ...
                    Using rdr = cmd.ExecuteReader()
                        ...
                        dgvPemasukanLainnya.DataSource = Nothing
                        dgvPemasukanLainnya.DataSource = dt
                        dgvPemasukanLainnya.Columns("kode_transaksi").HeaderText = "Kode Transaksi"
                        dgvPemasukanLainnya.Columns("nominal").HeaderText = "Nominal Transaksi"
                        dgvPemasukanLainnya.Columns("keterangan").HeaderText = "Keterangan"
                        dgvPemasukanLainnya.Columns("operator").HeaderText = "Operator"
                        dgvPemasukanLainnya.Columns("tanggal_transaksi").HeaderText = "Tanggal Transaksi"
                    End Using
                End Using
            End Using
    ...
    Private Sub FormPemasukanLainnya_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadData()
    End Sub
    ...
    Private Sub dgvPemasukanLainnya_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvPemasukanLainnya.CellContentClick
        ...
        ElseIf dgvPemasukanLainnya.Columns(e.ColumnIndex).Name = "hapus" Then
            ' Hapus data siswa
            If e.RowIndex >= 0 Then
                btnHapus.Enabled = True
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        Dim sql As String = "INSERT INTO transaksi(kode_transaksi, nominal, jenis_transaksi, keterangan, operator, tanggal_transaksi) VALUES (@kode_transaksi, @nominal, 'masuk', @keterangan, @operator, @tanggal_transaksi)"
        Try
            Using conn = DatabaseConnector.GetConnection()
                Using cmd = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
                    ...
                    conn.Open()
                    cmd.ExecuteNonQuery()
                    MessageBox.Show("Data Pemasukan Lain berhasil disimpan.", "Sukses", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End Using
            End Using
        Catch ex As Exception
